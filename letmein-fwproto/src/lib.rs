...
mod tests {
    ...
    fn test_msg_deserialize_invalid_operation() {
        // Build a valid Ack message then corrupt the operation field.
        let mut bytes = FirewallMessage::new_ack().msg_serialize().unwrap();
        bytes[FWMSG_OFFS_OPERATION] = 0xFF;
        bytes[FWMSG_OFFS_OPERATION + 1] = 0xFF;
    ...
    fn test_msg_deserialize_invalid_addr_type() {
        // Build a valid Install message then corrupt the addr_type field.
